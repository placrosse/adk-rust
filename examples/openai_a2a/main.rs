//! OpenAI A2A (Agent-to-Agent) Protocol Example
//!
//! This example demonstrates the A2A protocol integration with OpenAI for
//! agent-to-agent communication.
//!
//! It shows how to:
//! 1. Create an A2A server that exposes an OpenAI-powered agent via the A2A protocol
//! 2. Create a RemoteA2aAgent that can communicate with remote A2A agents
//!
//! To run this example:
//!   OPENAI_API_KEY=your_key cargo run --example openai_a2a --features openai
//!
...
async fn main() -> Result<()> {
    ...
    let weather_agent = LlmAgentBuilder::new("weather_agent")
        ...
        .instruction(
            "Answer questions about weather in cities. Provide helpful, accurate weather information. \
             Since you don't have real-time data, give general climate information for locations.",
        )
        .model(Arc::new(model))
        .build()?;

    println!("OpenAI A2A Protocol Example");
    println!("===========================\n");
